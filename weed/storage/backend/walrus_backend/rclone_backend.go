...
import (
	...
	"context"
	"fmt"
	"github.com/walrus/walrus/fs/config/configfile"
	...
	_ "github.com/walrus/walrus/backend/all"
	"github.com/walrus/walrus/fs"
	"github.com/walrus/walrus/fs/accounting"
	"github.com/walrus/walrus/fs/object"
...
func newWalrusBackendStorage(configuration backend.StringProperties, configPrefix string, id string) (s *WalrusBackendStorage, err error) {
	...
	if err != nil {
		glog.Errorf("failed to instantiate Walrus filesystem: %s", err)
		return
	}

	glog.V(0).Infof("created backend storage walrus.%s for remote name %s", s.id, s.remoteName)
	return
}
...
func (walrusBackendStorageFile WalrusBackendStorageFile) GetStat() (datSize int64, modTime time.Time, err error) {
	files := walrusBackendStorageFile.tierInfo.GetFiles()
